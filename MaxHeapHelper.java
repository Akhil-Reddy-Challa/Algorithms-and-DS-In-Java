...
public class MaxHeapHelper {
    ...
    public void remove() {
        /*
         * Goal: Removing the root(Highest Integer) node and make 2nd Highest int as the
    ...
    private int getLargerChildIndex(int index) {
        ...
        if (hasLeftChild(index) && values[leftChildIndex(index)] > values[index])
            highIndex = leftChildIndex(index);// We are not returning yet, because right child might be bigger
        if (hasRightChild(index) && values[rightChildIndex(index)] > values[leftChildIndex(index)]
                && values[rightChildIndex(index)] > values[index])
    ...
    private boolean ifParentIsValid(int index) {
        ...
        if (!hasLeftChild(index))
            return false;// Because insertion happens from left to right in HEAP, if a node does not have
                         // left child, then no right child would exist

        if (values[leftChildIndex(index)] >= values[index])
            return true;
        // Above says that left child is greater than root, so we return true to swap
        ...
        /*
         * At this point either 1) right,left child are lesser than root or 2) no right
         * child
         */
        return false;
