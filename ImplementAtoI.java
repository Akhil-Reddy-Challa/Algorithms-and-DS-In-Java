        ...
        for (var character : number.toCharArray()) {
            if (character != ' ') {
                ...
                if (ascii_of_char > 47 && ascii_of_char < 58) {
                    ...
                    break;
            } else if (first_non_whitespace_character)
                break;
        }
        if (count_of_integers > 9 && negative_number)
            converted_number = Integer.MIN_VALUE;
        else if (count_of_integers > 9 && !negative_number)
            converted_number = Integer.MAX_VALUE;
        else if (count_of_integers <= 9 && negative_number)
